...
if [ "$originalArgOne" = 'mongod' ]; then
	...
	file_env 'MONGO_DATABASE_NAME'
	file_env 'MONGO_READ_USER'
	file_env 'MONGO_READ_USER_PASSWORD'
	file_env 'MONGO_WRITE_USER'
	file_env 'MONGO_WRITE_USER_PASSWORD'
		...
		if [ "$MONGO_INITDB_ROOT_USERNAME" ] && [ "$MONGO_INITDB_ROOT_PASSWORD" ]; then
			...
			if [ "$MONGO_DATABASE_NAME" ]; then
				...
				if [ "$MONGO_READ_USER" ] && [ "$MONGO_READ_USER_PASSWORD" ]; then
					"${mongo[@]}" <<-RUJS
						db.createUser({
							...
							roles: [{role: 'read', db: '${MONGO_DATABASE_NAME}'}]
						})
					RUJS
					echo "INFO: created ${MONGO_READ_USER} user with specified password"
				fi
				if [ "$MONGO_WRITE_USER" ] && [ "$MONGO_WRITE_USER_PASSWORD" ]; then
					...
					WUJS
					echo "INFO: created ${MONGO_WRITE_USER} user with specified password"
				fi
			fi
		fi
